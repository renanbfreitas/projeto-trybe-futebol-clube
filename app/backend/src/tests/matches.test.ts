...
describe('1. Teste da rota matches', () => {
  /**
  ...
  it('1.2. Testando o metodo get rota matches inProgress === true', async () => {
    chaiHttpResponse = await chai
  ...
  it('1.3. Testando o metodo get rota matches inProgress === false', async () => {
    chaiHttpResponse = await chai
  ...
  it('1.4. Testando o metodo patch rota matches para finalizar matche', async () => {
    chaiHttpResponse = await chai
  ...
  it('1.5. Testando o metodo patch rota matches para finalizar matche token invalido', async () => {
    chaiHttpResponse = await chai
  ...
  it('1.6. Testando o metodo patch rota matches para editar matche', async () => {
    chaiHttpResponse = await chai
  ...
  it('1.7. Testando o metodo patch rota matches para criar matche id nao existe', async () => {
    chaiHttpResponse = await chai
  ...
  it('1.8. Testando o metodo patch rota matches para criar matche id iguais', async () => {
    chaiHttpResponse = await chai
       .request(app).post('/matches')
        .set('authorization', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwidXNlcm5hbWUiOiJBZG1pbiIsInJvbGUiOiJhZG1pbiIsInBhc3N3b3JkIjoiJDJhJDA4JHhpLkh4azFjekFPMG5aUi4uQjM5M3UxMGFFRDBSUTFOM1BBRVhRN0h4dExqS1BFWkJ1LlBXIiwiZW1haWwiOiJhZG1pbkBhZG1pbi5jb20iLCJpYXQiOjE2Nzc1OTA1NDF9.QXwSdQYZKCYzLWS7njCAU4Xf46KhRsm3hbCVVVsyNqI')
        .send({
  ...
  it('1.9. Testando o metodo patch rota matches para criar matche valida', async () => {
    chaiHttpResponse = await chai
       .request(app).post('/matches')
        .set('authorization', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwidXNlcm5hbWUiOiJBZG1pbiIsInJvbGUiOiJhZG1pbiIsInBhc3N3b3JkIjoiJDJhJDA4JHhpLkh4azFjekFPMG5aUi4uQjM5M3UxMGFFRDBSUTFOM1BBRVhRN0h4dExqS1BFWkJ1LlBXIiwiZW1haWwiOiJhZG1pbkBhZG1pbi5jb20iLCJpYXQiOjE2Nzc1OTA1NDF9.QXwSdQYZKCYzLWS7njCAU4Xf46KhRsm3hbCVVVsyNqI')
        .send({
